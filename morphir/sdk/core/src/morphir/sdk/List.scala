...
object List {
  ...
  @inline def all[A](predicate: A => Bool, xs: List[A]): Bool =
    xs.forall(predicate)

  @inline def any[A](predicate: A => Bool, xs: List[A]): Bool =
    xs.exists(predicate)
  ...
  @inline def drop[A](n: Int, xs: List[A]): List[A] = xs.drop(n.intValue)

  @inline def empty[A]: List[A] = Nil

  @inline def filter[A](f: A => Bool, xs: List[A]): List[A] =
    xs.filter(f)
  ...
  @inline def member[A, A1 >: A](candidate: A1, xs: List[A]): Bool =
    xs.contains(candidate)

  @inline def partition[A](f: A => Bool, xs: List[A]): (List[A], List[A]) =
    xs.partition(f)
  ...
  @inline def repeat[A](n: Int, elem: => A): List[A] =
    scala.List.fill(n.intValue)(elem)
  ...
  @inline def take[A](n: Int, xs: List[A]): List[A] = xs.take(n.intValue)
